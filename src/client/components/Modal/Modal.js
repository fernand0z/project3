...
const Tray = styled.div`
  ...
  transition: visibility 1s linear;
  animation: 1s
    ${props => keyframes`${props.show ? slideInRight : slideOutRight}`};
`;
...
class Modal extends React.PureComponent {
  ...
  render() {
    return createPortal(
      <Background show={this.props.show} onClick={this.handleClick}>
        <Tray show={this.props.show}>{this.props.children}</Tray>
      </Background>,
      this.el
    );
  }
